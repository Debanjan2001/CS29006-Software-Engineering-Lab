...
root = Tk()
root.configure(background='gray81')
class LeftFrame(Frame):
    def __init__(self,master=None,drawing_frame = None):
        super().__init__(master,bg="khaki1")
        self.master=master
        ...
        self.expr.grid(row=0,column=0,padx = 5,pady = 10)
        self.expression = Text(self,height=2,width=20,font=app_font)
        self.expression.grid(row=0,column=1,padx = 5,pady = 10)

        self.lb = Label(self,text = "Enter Lower Bound of Iteration : ",font=app_font)
        self.lb.grid(row=1,column=0,padx=5,pady=10)
        self.lbound = Text(self,height=2,width=20,font=app_font)
        self.lbound.grid(row=1,column=1,padx = 5,pady = 10)

        self.ub = Label(self,text = "Enter Upper Bound of Iteration : ",font=app_font)
        self.ub.grid(row=2,column=0,padx=5,pady=10)
        self.ubound = Text(self,height=2,width=20,font=app_font)
        self.ubound.grid(row=2,column=1,padx = 5,pady = 10)

        self.button = Button(self,text = "Create Plot",command=self.draw_plot,font=app_font,fg="white",bg="black")
        self.button.grid(row=3,column=0,padx=15)  


        # bl0 = Label(self,text="",bg="khaki1").grid(row=4,column=0,sticky="w")
        # bl1 = Label(self,text="aaa",bg="khaki1").grid(row=4,column=1,sticky="w")
        # bl2 = Label(self,text="",bg="khaki1").grid(row=4,column=2,sticky="w")
    ...
    def draw_plot(self):
        ...
        h = self.drawing_frame.winfo_height()
        w = self.drawing_frame.winfo_width()
        self.master.geometry(f"{2*w+200}x{h+100}")
...
class RightFrame(Frame):
    def __init__(self,master=None):
        ...
        self.master=master
        self.grid(row=0,column=1,sticky="nsew",padx=50,pady=50)
